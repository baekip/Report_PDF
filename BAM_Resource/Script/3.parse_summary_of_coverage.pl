...
foreach ( @list_delivery_tbi_id ){
	...
	my $sequence_read = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 2`;
        ...
        my $bed_size = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 15`;
        my $raw_sequence_depth = $sequence_base/$bed_size;
        $raw_sequence_depth = &RoundXL($raw_sequence_depth,2);

        my $ontarget_depth_mean = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 13`;
        chomp ($ontarget_depth_mean);
        my $ontarget_depth_std = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 14`;
        chomp ($ontarget_depth_std);
        $ontarget_depth_std = &RoundXL($ontarget_depth_std,2);

        my $coverage_1x_rate = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 16`;
	chomp($coverage_1x_rate);
	my $coverage_5x_rate = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 17`;
	chomp($coverage_5x_rate);
	my $coverage_10x_rate = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 18`;
	chomp($coverage_10x_rate);
	my $coverage_20x_rate = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 19`;
	chomp($coverage_20x_rate);
	my $coverage_50x_rate = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 20`;
