...
#        chomp($average_depth);
        ...
        my $average_depth = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 13`;
        chomp ($average_depth);
...
#        $average_depth = &RoundXL ($average_depth, 2);
	...
	print "$delivery_id\t$tbi_id\t$average_depth\t$coverage_1x_rate\t$coverage_5x_rate\t$coverage_10x_rate\t$coverage_20x_rate\t$coverage_50x_rate\n";
