...
if ( @delivery_tbi_list != 1) {
    print "[[12],[],[],[100,0,100]]\n";
    print "Sample ID\tDistribution of Allele Frequency\tDelivery ID\tDistribution of Allele Frequency\n"; 
    if($rev == 0){
        for(my $i=0; $i<@delivery_tbi_list; $i=$i+2){
            ...
            print "$delivery_id_1\t$allele_fig_name_1\t$delivery_id_2\t$allele_fig_name_2\n";
        }
    }elsif ($rev == 1){ 
