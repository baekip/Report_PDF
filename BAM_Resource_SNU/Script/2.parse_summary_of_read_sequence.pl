...
print "Delivery ID\tSample ID\tSequence read\t".
	"Deduplication read\\n(%)\t".
	"Mapping read\\n(%)\t".
	"On target read\\n(%)\n";
foreach ( @list_delivery_tbi_id ){
	...
	my $deduplication_read_rate = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 9`;
	chomp($deduplication_read_rate);
	my $mapping_read_rate = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 11`;
	chomp($mapping_read_rate);
	my $on_target_read_rate = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 20`;
