...
foreach ( @list_delivery_tbi_id ){
	...
	my $sequence_read = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 2`;
        ...
        my $bed_size = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 23`;
        my $raw_sequence_depth = $sequence_base/$bed_size;
        $raw_sequence_depth = &RoundXL($raw_sequence_depth,2);

        my $ontarget_depth_mean = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 21`;
        chomp ($ontarget_depth_mean);
        my $ontarget_depth_std = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 22`;
	...
	print "$delivery_id\t$raw_sequence_depth\t$ontarget_depth_mean\\n($ontarget_depth_std)\t$coverage_1x_rate\t$coverage_4x_rate\t$coverage_8x_rate\t$coverage_20x_rate\t$coverage_50x_rate\n";
