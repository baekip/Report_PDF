...
#        $sequence_read = num($sequence_read);
	...
	my $on_target_depth_mean = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 21`;
	chomp($on_target_depth_mean);

	my $on_target_depth_std = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 22`;
