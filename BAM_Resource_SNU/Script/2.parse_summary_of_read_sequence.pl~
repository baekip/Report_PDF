...
foreach ( @list_delivery_tbi_id ){
	my ($delivery_id,$tbi_id,$type_id) = split /\:/, $_;
        ...
        my $deduplicated_read = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 3 `;
        chomp $deduplicated_read;
        $deduplicated_read = num($deduplicated_read);
	my $deduplicated_read_rate = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 4 | head -n 1`;
	chomp($deduplicated_read_rate);

        my $mapping_read = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 5 `;
        chomp $mapping_read;
	...
	chomp($mapping_read_rate);
        ...
        my $unique_read = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 9 `;
        chomp $unique_read;
        $unique_read = num($unique_read);
	my $unique_read_rate = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 10 | head -n 1`;
	chomp($unique_read_rate);

        my $on_target_read = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 11`;
