...
my $snpeff_pathogenic_path = "$project_path/result/14-2_snpeff_pathogenic_isolate_run/";
...
#    print $list_delivery_id[$i]."\n";
    ...
    my $in_pathogenic_snpeff_tsv = "$snpeff_pathogenic_path/".$tbi_id."/".$tbi_id.".pathogenic.BOTH.snpeff.isoform.tsv";
#    print $in_pathogenic_snpeff_tsv; 
...
#    my $output_table = "3.4.1 ".$i."_table_01.txt";
    ...
    print $fh_table "[[10],[],[],[0,50,0,0,40,85,40,135]]\n";
    print $fh_table "Clinical\\neffect\tGene\tDNA\\nchange\tProtein\\nchange\tGenotype\tInhertance\tVariant_Depth\\nALT/REF\tDisease\n";
...
#        print $col_contents[30];
        my $gene = $col_contents[11]; my $dna_change = $col_contents[9]; my $protein_change = $col_contents[10];
#        my $variant_count = comma($col_contents[6]);
	my $variant_count = $col_contents[6];
        my $depth = $col_contents[6]; my $exonic_effect = $col_contents[3];
	my $inhertance = $col_contents[22];
	my $genotype = $col_contents[5];

        my $clnsig = $col_contents[12];
        my $clndbn = $col_contents[17];
...
#            open (my $fh_table, '>', $output_table) or die;
            for (my $j=0; $j<@criteria; $j++){ 
                ...
                #}
                if ($clinical_effect eq "Pathogenic"){ 
...
#                    print $fh_table "$clinical_effect\t$gene\t$dna_change\t$protein_change\t$variant_count\t$depth\t$exonic_effect\t$disease\n";
                    print $fh_table "$clinical_effect\t$gene\t$dna_change\t$protein_change\t$genotype\t$inhertance\t$variant_count\t$disease\n";
                #print $value."\t".$disease."\t";
                }elsif ($clinical_effect eq "Likely pathogenic"){
                    $clinical_effect = "Likely pathogenic";
                    if ($disease =~ /\\x2c/){
                        $disease =~ s/\\x2c//g;
                    }
                    print $fh_table "$clinical_effect\t$gene\t$dna_change\t$protein_change\t$genotype\t$inhertance\t$variant_count\t$disease\n"; 
