...
=cut
    ...
    my $sample_id = `cat $experiment_file | grep \"$tbi_id\" | cut -f 6`;
    chomp ($sample_id);
    my $sample_type = `cat $experiment_file | grep \"$tbi_id\" | cut -f 9`;
    chomp ($sample_type);
    my $q_bit = `cat $experiment_file | grep \"$tbi_id\" | cut -f 12`;
    chomp ($q_bit);
    my $total_volume = `cat $experiment_file | grep \"$tbi_id\" | cut -f 14`;
    chomp ($total_volume);
    my $acid_ratio = `cat $experiment_file | grep \"$tbi_id\" | cut -f 16`;
    chomp ($acid_ratio);
    my $protein_ratio = `cat $experiment_file | grep \"$tbi_id\" | cut -f 17`;
    chomp ($protein_ratio);

    print "$sample_id\\n($tbi_id)\t$sample_type\t$q_bit\t$total_volume\t$acid_ratio\t$protein_ratio\n";
