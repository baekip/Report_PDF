...
foreach (@sample_tbi_list){
    ...
    my $deduplicated_read = `cat $alignment_xls | grep \"^$tbi_id\" | cut -f 3 `;
    chomp $deduplicated_read;
    $deduplicated_read = num($deduplicated_read);

    my $mapping_read = `cat $alignment_xls | grep \"^$tbi_id\" | cut -f 5 `;
    chomp $mapping_read;
    $mapping_read = num($mapping_read);

    my $unique_read = `cat $alignment_xls | grep \"^$tbi_id\" | cut -f 9 `;
    chomp $unique_read;
    $unique_read = num($unique_read);

    my $on_target_read = `cat $alignment_xls | grep \"^$tbi_id\" | cut -f 11`;
    chomp $on_target_read;
    $on_target_read = num($on_target_read);


    print "$delivery_id\t$tbi_id\t$sequence_read\t$deduplicated_read\t$mapping_read\t$unique_read\t$on_target_read\n";}
