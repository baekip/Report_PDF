...
open my $fh_stat, '<:encoding(UTF-8)', $alignment_statistics_xls or die;
while (my $row_stat = <$fh_stat>){
    chomp $row_stat;
    my @row_clumn = split /\t/, $row_stat;
    print $row_clumn[0]."\n";
    #push my @stat_array, $row_stat ;
    #print $stat_array[0];
}
...
foreach ( @list_delivery_tbi_id ){
	...
	chomp($sequence_read);
        ...
        my $deduplication_read_rate = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 4`;
	chomp($deduplication_read_rate);

        my $mapping_read_rate = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 6`;
	chomp($mapping_read_rate);

        my $on_target_read_rate = `cat $alignment_statistics_xls | grep \"^$tbi_id\" | cut -f 12`;
	chomp($on_target_read_rate);
 
	print "$delivery_id\t$tbi_id\t$sequence_read\t$deduplication_read_rate\t$mapping_read_rate\t$on_target_read_rate\n";
