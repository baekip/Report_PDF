...
$DOWNSTREAM = check_null($DOWNSTREAM);
$EXON = check_null($EXON);
$INTERGENIC = check_null($INTERGENIC);
$INTRON = check_null($INTRON);
$SPLICE_SITE_ACCEPTOR = check_null($SPLICE_SITE_ACCEPTOR);
$SPLICE_SITE_DONOR = check_null($SPLICE_SITE_DONOR);
$SPLICE_SITE_REGION = check_null($SPLICE_SITE_REGION);
$TRANSCRIPT = check_null($TRANSCRIPT);
$UPSTREAM = check_null($UPSTREAM);
$UTR_3_PRIME = check_null($UTR_3_PRIME);
$UTR_5_PRIME = check_null($UTR_5_PRIME);
