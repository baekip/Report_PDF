...
my $Sequencing_Statistics_Result_xls = "$project_path/statQCResult/Sequencing_Statistics_Result.xls";
checkFile( $Sequencing_Statistics_Result_xls );

my %hash_sample;
print "[[12],[],[],[100]]\n";
print "Delivery ID\\n(Sample ID)\tTotal reads\tTotal\\nyield(bp)\tTotal\\nyield(Gbp)\tGC\\nPercent\t% of >= Q30 Bases(PF)\n";
foreach ( @list_delivery_tbi_id ){
	my ($delivery_id,$tbi_id,$type_id) = split /\:/, $_;

	# Total reads 
	my $total_reads = `cat $Sequencing_Statistics_Result_xls | grep \"^$tbi_id\" | cut -f 2`;
	chomp($total_reads);
    	$total_reads = num($total_reads);

        my $total_bp = `cat $Sequencing_Statistics_Result_xls | grep \"^$tbi_id\" | cut -f 3`;
        chomp($total_bp);
        $total_bp = num($total_bp);
#	my $index = `cat $Sequencing_Statistics_Result_xls | grep \"^$tbi_id\" | cut -f 2`;
#	chomp($index);

	my $total_yield = `cat $Sequencing_Statistics_Result_xls | grep \"^$tbi_id\" | cut -f 3`;
	chomp($total_yield);
	$total_yield = changeGbp($total_yield);

	my $GC_contents = `cat $Sequencing_Statistics_Result_xls | grep \"^$tbi_id\" | cut -f 5`;
	chomp($GC_contents);
	$GC_contents = changeGbp($GC_contents);
	#$GC_contents = num($GC_contents);

	my $GC_percent = `cat $Sequencing_Statistics_Result_xls | grep \"^$tbi_id\" | cut -f 6`;
	chomp($GC_percent);	

	my $q30_base_pf = `cat $Sequencing_Statistics_Result_xls | grep \"^$tbi_id\" | cut -f 14`;
	chomp($q30_base_pf);

#	my $q20_base_pf = `cat $Sequencing_Statistics_Result_xls | grep \"^$tbi_id\" | cut -f 17`;
#	chomp($q20_base_pf);
	
	print "$delivery_id\\n($tbi_id)\t$total_reads\t$total_bp\t$total_yield\t$GC_percent\t$q30_base_pf\n";
}
